...
export function PricingCard({
  ...
  return (
    <div
      key={plan.name}
      className={cn(
        "relative flex w-full flex-col rounded-lg border bg-black/30",
        className,
      )}
      {...props}
    >
      ...
      <div
        className={cn(
          "bg-muted/20 relative rounded-t-lg border-b p-4",
      ...
      >
        {plan.features.map((feature, index) => (
          <div key={index} className="flex items-center gap-2 bg-black/30">
      ...
      <div
        className={cn(
          "mt-auto relative w-full border-t p-3",
          plan.highlighted && "bg-muted/40",
        )}
      >
        {plan.isFree ? (
          ...
          <Button className="w-full" variant={plan.highlighted ? "default" : "outline"} asChild>
            <Link href={plan.btn.href as never}>{plan.btn.text}</Link>
          </Button>
        )}
      </div>
    </div>
...
export function BorderTrail({
  ...
  return (
    <div className="pointer-events-none absolute inset-0 rounded-[inherit] border border-transparent [mask-clip:padding-box,border-box] mask-[linear-gradient(transparent,transparent),linear-gradient(#000,#000)] mask-intersect">
      <motion.div
